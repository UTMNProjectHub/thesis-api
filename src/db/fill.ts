...
async function createTrueFalseQuestion(quizId: string, _themeName: string) {
	const questionTypes = [
		{
...
async function createShortAnswerQuestion(quizId: string, _themeName: string) {
	const questionTypes = [
		{
...
async function createMatchingQuestion(quizId: string, _themeName: string) {
	const [question] = await db
		.insert(questions)
		.values({
			type: "matching",
